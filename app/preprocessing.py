...
def csv_to_db(file_path):
    ...
    path_prefix = "/".join(file_path.split('/')[:-1])
    new_file_name = f"{file_path.split('/')[-1].split('.')[0]}.db"
    ...
    cursor = conn.cursor()
    drop_table_query = "DROP TABLE IF EXISTS my_table;"
    cursor.execute(drop_table_query)

    # print(table_definition)

    create_table_query = f"CREATE TABLE my_table ({table_definition})"
    ...
    for _, row in df.iterrows():
        if not pd.isnull(row[0]):
            cursor.execute(insert_query.format(columns=",\n".join(
                formatted_names), placeholders=placeholders), row)
